    ...
    for x in range(len(map)):
        for y in range(len(map[0])):
            ...
            if "fridge" in map[x][y]:
                fridge = [y,x]
    policy1 = {"oven": oven, "counter": counter, "door": door, "cupboards": [tool, cupboard], "fridge": fridge, "target": "tool"}
    policy2 = {"oven": oven, "counter": counter, "door": door, "cupboards": [cupboard, tool], "fridge": fridge, "target": "plate"}
...
def train(filename, seed, exp_type):
    ...
    with open(filename, "w") as csvfile:
        ...
        for j, goal in enumerate(tasks):
            ...
            policy2 = BaseEmpathic(alpha=1.0, gamma=1.0, epsilon=0.5,
                                       default_q=DEFAULT_Q, num_actions=5, rng=rng1, others_policy=humans_policy1, others_init=[[1,3]], others_dist=[0.5, 0.5], penalty=-2*EPISODE_LENGTH, others_alpha=[4.01, 4.01], objects=OBJECTS1, problem_mode=problem_mode)
            ...
            policy3 = BaseEmpathic(alpha=1.0, gamma=1.0, epsilon=0.5,
                                       default_q=DEFAULT_Q, num_actions=5, rng=rng3, others_policy=humans_policy2, others_init=[[1,3]], others_dist=[0.5, 0.5], penalty=-2*EPISODE_LENGTH, others_alpha=[4.01, 4.01], objects=OBJECTS1, problem_mode=problem_mode, full_observability = True)
            ...
            try:
                ...
                if exp_type != "C":
                    ...
                    print("Human:")
                    humans, _ = generate_policies(map_fn, exp_type)
                    evaluate_human([last_state.plate_x, last_state.plate_y], [1, 3], humans[0], exp_type)
                    print("-----------------")
                    print("Our Robot:")
                    base_step2, last_state2 = evaluate_agent(env2, policy2, reward2, init2)
                    print("Total Reward = ", base_step2)
                    ...
                    print("Human:")
                    humans, _ = generate_policies(map_fn, exp_type)
                    evaluate_human([last_state2.plate_x, last_state2.plate_y], [1, 3], humans[0], exp_type)
                    print("-----------------")
                    print("Robot with Full Observability Assumption:")
                    base_step3, last_state3 = evaluate_agent(env3, policy3, reward3, init3)
                    print("Total Reward = ", base_step3)
                    ...
                    print("Human:")
                    humans, _ = generate_policies(map_fn, exp_type)
                    evaluate_human([last_state3.plate_x, last_state3.plate_y], [1, 3], humans[0], exp_type)
                    print("-----------------")
